...
class TestDropblockActual(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.conflict_file = self.temp_dir / "test_file (user's conflicted copy 2024-01-01).txt"
        self.conflict_file.write_text("conflict content")
        
        self.conflict_folder = self.temp_dir / "test_folder (conflicted copy 2024-01-01)"
    ...
    def test_conflict_detection_real(self):
        ...
        self.assertEqual(conflicts[0].name, "test_file (user's conflicted copy 2024-01-01).txt")
        
        folder_conflicts = ignorer.find_conflicts(self.test_folder)
        self.assertEqual(len(folder_conflicts), 1)
        self.assertEqual(folder_conflicts[0].name, "test_folder (conflicted copy 2024-01-01)")
    ...
    def test_cross_platform_ignore_real(self):
        ...
        if success:
            ...
            print(f"Cross-platform ignore failed on {platform.system()} (expected in test env)")
